...
'pole', 'traffic sign', 'traffic light', 'vegetation', 'terrain',
'sky', 'person', 'rider', 'car', 'truck',
'bus', 'on rails', 'motorcycle', 'bycycle'
...
model_dir = sys.path[0]
tf.gfile.MakeDirs(model_dir)

#download_path = os.path.join(model_dir, _TARBALL_NAME)
download_path = os.path.join(model_dir, 'deeplabv3_mnv2_cityscapes_train_2018_02_05.tar.gz')
...
class SemanticSegmentation:
    ...
    def image_callback(self, data):
        try:
            ...
            seg_image = get_dataset_colormap.label_to_color_image(seg_map, get_dataset_colormap.get_cityscapes_name()).astype(np.uint8)
